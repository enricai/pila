...
destroy-on-exit); repo seeding (git clone + rsync uncommitted delta) is
implemented by `scripts/remote/seed-repo.sh` (see below). Worker auth
forwarding is handled by a subsequent feat-remote-* subtask.
