"""Tests for `list_runs()` — the `centella --list` rendering function.

Behavioral tests use `tmp_path` for filesystem isolation. The function
reads `.centella/runs/*/state.json` (via discover_runs) and overlays
`.centella/runs/*/run.json` for status, then renders a sortable table
...
def test_list_runs_empty(centella, tmp_path, capsys):
    centella.list_runs(tmp_path)
    out = capsys.readouterr().out
    assert "no runs" in out


def test_list_runs_renders_table_header(centella, tmp_path, capsys):
...
def test_list_runs_in_progress_status(centella, tmp_path, capsys):
...
def test_list_runs_done_pushed_pr_status(centella, tmp_path, capsys):
    _make_run(tmp_path, "feat-a-aaaaaa",
              {"started_at": "2026-05-26T10:00:00+00:00", "task": "x"},
              run_json={
...
def test_list_runs_push_failed_status(centella, tmp_path, capsys):
...
def test_list_runs_sorted_newest_first(centella, tmp_path, capsys):
...
def test_list_runs_corrupt_sidecar(centella, tmp_path, capsys):
...
def test_list_runs_malformed_run_json_treated_as_missing(centella, tmp_path, capsys):
...
def test_list_runs_skips_bootstrap_dirs(centella, tmp_path, capsys):
