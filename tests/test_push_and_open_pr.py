...
def test_push_and_open_pr_exists(centella):
    """The helper is exported at module scope."""
    assert callable(getattr(centella, "push_and_open_pr", None))


def test_push_and_open_pr_uses_compute_run_branch(centella):
    """The function derives the branch name from compute_run_branch(st.run_id),
    not by hardcoding `centella/<something>`. Pin so a refactor doesn't
...
def test_push_and_open_pr_uses_compose_pr_body(centella):
...
def test_push_and_open_pr_writes_run_json_on_success(centella):
...
def test_push_and_open_pr_writes_run_json_on_failure(centella):
...
def test_push_and_open_pr_passes_no_verify(centella):
...
def test_push_and_open_pr_uses_origin(centella):
...
def test_phase_finalize_takes_no_push_and_no_verify(centella):
    sig = inspect.signature(centella.phase_finalize)
    assert "no_push" in sig.parameters
    assert "no_verify" in sig.parameters


def test_phase_finalize_calls_push_and_open_pr_unless_no_push(centella):
...
def test_orchestrate_writes_initial_run_json_after_rename():
    ...
    `centella --list` (commit 5) can't enumerate it.
    ...
    for field in ("run_id=", "branch=", "working_branch=",
        ...
        assert field in block, (
            ...
            "Fields needed for centella --list: run_id, branch, "
...
def test_no_verify_only_affects_push(centella):
