...
(pila_root, run_id), and `State.rename_to()` for the bootstrap →
final-run-id rename.
...
def test_state_path_is_per_run(pila, tmp_path):
    """state.json lives at pila_root/runs/<run-id>/state.json."""
    st = pila.State(tmp_path, "feat-foo-abc123")
    ...
    assert st.pila_root == tmp_path


def test_state_save_writes_to_per_run_dir(pila, tmp_path):
...
def test_state_load_reads_from_per_run_dir(pila, tmp_path):
    import json
...
def test_state_load_returns_false_when_absent(pila, tmp_path):
...
def test_two_states_disjoint_paths(pila, tmp_path):
    ...
    # pila_root — they're siblings.
    assert sa.run_dir.parent == sb.run_dir.parent == tmp_path / "runs"


def test_two_states_save_independently(pila, tmp_path):
...
def test_rename_to_moves_run_dir(pila, tmp_path):
    ...
    st = pila.State(tmp_path, "_bootstrap-abcdef")
    st.data = {"task": "x"}
    st.save()
...
def test_rename_to_preserves_state_data(pila, tmp_path):
...
def test_rename_to_dies_on_collision(pila, tmp_path):
...
def test_rename_to_preserves_sub_directories(pila, tmp_path):
