...
centella runs in the same repository have completely disjoint state.
...
def test_disjoint_state_paths(centella, tmp_path):
...
def test_disjoint_run_dirs(centella, tmp_path):
    """Run dirs are siblings under centella_root/runs/; nothing inside
...
def test_disjoint_subpaths(centella, tmp_path):
    """The per-run subdirectories that centella code uses
    (centella_dir / 'worktrees', '/criteria', '/logs', '/subtasks',
...
def test_concurrent_saves_do_not_clobber(centella, tmp_path):
...
def test_interleaved_saves_do_not_clobber(centella, tmp_path):
...
def test_same_subtask_id_in_different_runs(centella, tmp_path):
