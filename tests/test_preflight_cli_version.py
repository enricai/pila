...
Centella's worker invocation requires `--json-schema`, which is only
...
- Source-text pins on `centella.py` enforce the placement contract: the
...
def test_parse_version_typical_native_output(centella):
    assert centella._parse_claude_version("2.1.150 (Claude Code)\n") == (2, 1, 150)


def test_parse_version_at_minimum(centella):
    assert centella._parse_claude_version("2.1.22 (Claude Code)") == (2, 1, 22)


def test_parse_version_old_v1(centella):
    assert centella._parse_claude_version("1.0.72 (Claude Code)") == (1, 0, 72)


def test_parse_version_empty_string(centella):
    assert centella._parse_claude_version("") is None


def test_parse_version_none(centella):
    assert centella._parse_claude_version(None) is None


def test_parse_version_unrecognized_text(centella):
    ...
    assert centella._parse_claude_version("hello world") is None


def test_parse_version_extra_whitespace(centella):
    assert centella._parse_claude_version("   2.1.143  ") == (2, 1, 143)


# --- ordering comparison sanity (the actual gate logic) -------------------

def test_min_claude_cli_constant_value(centella):
    ...
    assert centella.MIN_CLAUDE_CLI == (2, 1, 22)


def test_old_version_below_floor(centella):
    assert centella._parse_claude_version("1.0.72") < centella.MIN_CLAUDE_CLI


def test_boundary_version_at_floor(centella):
    assert centella._parse_claude_version("2.1.22") == centella.MIN_CLAUDE_CLI


def test_new_version_above_floor(centella):
    assert centella._parse_claude_version("2.1.150") > centella.MIN_CLAUDE_CLI
...
def test_min_claude_cli_defined_in_source():
    ...
    test code reads it via the `centella` fixture."""
    src = CENTELLA_PY.read_text()
