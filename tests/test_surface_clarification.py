...
returns True; non-TTY path writes .pila/pending-clarifications.json
...
def test_tty_path_stores_answer_under_question_id(pila, state, monkeypatch):
    ...
    result = pila.surface_clarification(
        "auth-001", q, "/tmp/checkpoint.md", state)
    assert result is True
    assert state.data["answers"]["auth-001-q1"] == "preserve compat"


def test_tty_path_strips_whitespace(pila, state, monkeypatch):
    ...
    q = _question()
    pila.surface_clarification("feat-001", q, "/tmp/cp.md", state)
    assert state.data["answers"][q["id"]] == "break compat"


def test_tty_path_empty_input_stores_empty_string(pila, state, monkeypatch):
...
def test_tty_path_persists_state_to_disk(pila, state, monkeypatch):
    ...
    q = _question("test-q1")
    pila.surface_clarification("test-sid", q, "/tmp/cp.md", state)
...
def test_non_tty_writes_pending_clarifications_json(pila, state,
                                                    monkeypatch):
...
def test_non_tty_saves_state_before_exiting(pila, state, monkeypatch):
    ...
    with pytest.raises(SystemExit):
        pila.surface_clarification("sid", q, "/cp.md", state)
    # state.json on disk reflects the in-memory state at exit time.
    on_disk = json.loads(state.path.read_text())
    assert on_disk["answers"]["earlier-answer"] == "from before"


def test_non_tty_exit_code_is_exit_needs_answers(pila, state, monkeypatch):
    ...
    assert pila.EXIT_NEEDS_ANSWERS == 10


def test_non_tty_does_not_modify_answers(pila, state, monkeypatch):
