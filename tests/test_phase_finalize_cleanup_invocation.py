...
continues, while every worktree under .pila/runs/<id>/worktrees/
survives on disk.
...
def test_phase_finalize_invokes_cleanup_with_run_id(pila):
    ...
    assert 'run_script("cleanup.sh", "--run-id", st.run_id, "--subtask-branches")' in src, (
        ...
        "clutter post-finalize while keeping pila/runs/<id> as the "
        "PR head."
    )


def test_phase_finalize_does_not_use_bare_cleanup(pila):
